...
int proc = 0;                       /* proc of the current process */
int nprocs = 0;                /* Number of processes in communicator */


int iLeft; // array bounds
int iRight;
int eachNode;  // array nproc
...
int main(int argc, char *argv[])
{
    ...
    if (!(imax % nprocs == 0)) {
        if (proc == 0){
            // Imax is not divisible by process count
            MPI_Abort(MPI_COMM_WORLD, 1);
        }
    }

    eachNode = imax / nprocs;
    
    // node's array left bound
    iLeft = 1 + (eachNode * proc);

    // node's array right bound (no bigger than imax)
    
    iRight = min(imax, eachNode * (proc + 1));
...
static void print_help(void)
{
    ...
    fprintf(stderr, "  -d, --del-t=DELT      Set the simulation timestep nproc\n");
